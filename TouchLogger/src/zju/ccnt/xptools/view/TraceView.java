...
public class TraceView extends View {
	...
	public static class PointerState {
		...
		public void addParam(float velocityX, float velocityY, long time,
				float pressure) {
	...
	protected void onDraw(Canvas canvas) {
		...
		synchronized (mPointers) {
			...
			if (mActivePointerId >= 0) {
				...
				canvas.drawText(
						mText.clear().append("P: ").append(mCurNumPointers)
				...
				if ((mCurDown && ps.mCurDown) || N == 0) {
					...
					canvas.drawText(
							mText.clear().append("X: ").append(ps.mCoords.x, 1)
									.toString(), 1 + itemW, base, mTextPaint);
					canvas.drawRect(itemW * 2, 0, (itemW * 3) - 1, bottom,
							mTextBackgroundPaint);
					canvas.drawText(
							mText.clear().append("Y: ").append(ps.mCoords.y, 1)
									.toString(), 1 + itemW * 2, base,
							mTextPaint);
				} else {
					...
					canvas.drawRect(
							...
							Math.abs(dx) < mVC.getScaledTouchSlop() ? mTextBackgroundPaint
									: mTextLevelPaint);
					canvas.drawText(mText.clear().append("dX: ").append(dx, 1)
							.toString(), 1 + itemW, base, mTextPaint);
					canvas.drawRect(
							itemW * 2,
							0,
							...
							Math.abs(dy) < mVC.getScaledTouchSlop() ? mTextBackgroundPaint
									: mTextLevelPaint);
					canvas.drawText(mText.clear().append("dY: ").append(dy, 1)
			...
			for (int p = 0; p < NP; p++) {
				...
				if (drawn) {
					...
					for (int i = -ESTIMATE_PAST_POINTS + 1; i <= ESTIMATE_FUTURE_POINTS; i++) {
						float x = ps.mEstimator
								.estimateX(i * ESTIMATE_INTERVAL);
						float y = ps.mEstimator
								.estimateY(i * ESTIMATE_INTERVAL);
				...
				if (mCurDown && ps.mCurDown) {
					...
					if (ps.mToolType == MotionEvent.TOOL_TYPE_STYLUS
						...
						canvas.drawLine(ps.mCoords.x, ps.mCoords.y,
								ps.mCoords.x + orientationVectorX, ps.mCoords.y
					...
					canvas.drawCircle(ps.mCoords.x + orientationVectorX
							* tiltScale, ps.mCoords.y + orientationVectorY
							* tiltScale, 3.0f, mPaint);
				}
			}
		}
	}
	...
	private void logMotionEvent(String type, MotionEvent event) {
		...
		for (int i = 0; i < NI; i++) {
			...
			logCoords(type, action, i, mTempCoords, id, event.getToolType(i),
					event.getButtonState());
		}
	}

	private void logCoords(String type, int action, int index,
			MotionEvent.PointerCoords coords, int id, int toolType,
			int buttonState) {
		final String prefix;
		...
		Log.i(TAG,
				mText.clear()
						...
						.append(coords.y, 3)
						.append(") Pressure=")
						.append(coords.pressure, 3)
						.append(" Size=")
						.append(coords.size, 3)
						.append(" TouchMajor=")
						.append(coords.touchMajor, 3)
						.append(" TouchMinor=")
						.append(coords.touchMinor, 3)
						.append(" ToolMajor=")
						.append(coords.toolMajor, 3)
						.append(" ToolMinor=")
						.append(coords.toolMinor, 3)
						.append(" Orientation=")
						.append((float) (coords.orientation * 180 / Math.PI), 1)
						.append("deg")
						.append(" Tilt=")
						.append((float) (coords
								.getAxisValue(MotionEvent.AXIS_TILT) * 180 / Math.PI),
								1)
						.append("deg")
						.append(" Distance=")
						.append(coords.getAxisValue(MotionEvent.AXIS_DISTANCE),
								1)
						.append(" VScroll=")
						.append(coords.getAxisValue(MotionEvent.AXIS_VSCROLL),
								1)
						.append(" HScroll=")
						.append(coords.getAxisValue(MotionEvent.AXIS_HSCROLL),
								1).append(" ToolType=")
						.append(MotionEvent.toolTypeToString(toolType))
						.append(" ButtonState=")
						.append(MotionEvent.buttonStateToString(buttonState))
						.toString());
	}
			...
			for (int historyPos = 0; historyPos < N; historyPos++) {
				for (int i = 0; i < NI; i++) {
					...
					if (ps != null) {
						...
						ps.addParam(-1, -1, event.getHistoricalEventTime(i),
								event.getHistoricalPressure(i, historyPos));
					}
				}
			}
			for (int i = 0; i < NI; i++) {
				...
				if (ps != null) {
					ps.addTrace(coords.x, coords.y);
					ps.addParam(mVelocity.getXVelocity(id),
							mVelocity.getYVelocity(id), event.getEventTime(),
							event.getPressure(i));
			...
			if (action == MotionEvent.ACTION_UP
				...
				if (action == MotionEvent.ACTION_UP
						...
						public void run() {
							...
							for (int p = 0; p < fNP; p++) {
								...
								for (int i = 0; i < ps1.mTraceCount; i++) {
									PointData pd = new TouchDataModel.PointData();
									...
									if (i != (ps1.mTraceCount - 1)) {
										...
										// 预测坐标值
										pd.extimateX = ps1.mEstimator
												.estimateX(ESTIMATE_INTERVAL);
										pd.extimateY = ps1.mEstimator
												.estimateY(ESTIMATE_INTERVAL);
									}
									pointDatas.add(pd);
									Log.i("xptools", "x:" + pd.x);
									Log.i("xptools", "y:" + pd.y);
									Log.i("xptools", "xVelocity:"
											+ pd.xVelocity);
									Log.i("xptools", "yVelocity:"
											+ pd.yVelocity);
									Log.i("xptools", "pressure:" + pd.pressure);
									Log.i("xptools", "extimateX:"
											+ pd.extimateX);
									Log.i("xptools", "extimateY:"
											+ pd.extimateY);
								}
								Log.i("xptools", "TraceCount:"
										+ ps1.mTraceCount);
							...
							String packageName = info.topActivity
									.getPackageName();
					...
					if (mActivePointerId == id) {
						mActivePointerId = event.getPointerId(index == 0 ? 1
								: 0);
					}
